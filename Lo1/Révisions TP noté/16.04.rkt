...
; Q6
(define (chaine f)
  (cond [(atomicFbf? f) (symbol->string f)]
        [(negFbf? f) (string-append (symbol->string (conRac f)) (chaine (fils f)))]
        [else (string-append
...
(define (consequance2? F1 F2)
  (andmap (lambda(I) (if
                      (and (= (valV F1 I) 1) (= (valV F2 I) 0))
                       #f #t ))
          (ensInt (set-union (ensSP F1) (ensSP F2))) ))

;Q19
(define (ensSPallFbf ensF) 
  (if (set-empty? ensF) '()
...
(define (oteImp f)
  (cond [(atomicFbf? f) f]
        ...
        [else (let ((fg (oteImp (filsG f))) (fd (oteImp (filsD f)))) (list 'v (list '! fg) fd))] ))
...
;(resolvante c2 c3)
;(allResolvante fc1)
;(clause fc3) ; NE MARCHE PAS TOUJOURS
