...
(display "\n=>q26\n")
;(define (ssFbf fbf)
;  (letrec [(fonction (lambda (fbf liste)
;                       (cond [(null? fbf) liste]
;                             [(symbProp? fbf) (cons fbf liste)]
;                             [(atomicFbf? fbf) liste]
;                             [(neg? fbf) (fonction (fils fbf))]
;                             [else (fonction (append
;                                              (fonction (filsG fbf) liste)
;                                              (fonction (filsD fbf) liste) ))])))]
;    (fonction fbf '()) ))

(define (ssFbf fbf)
  (letrec [(fonction (lambda (fbf liste)
                       (cond [(null? fbf) liste]
                             [(atomicFbf? fbf) (cons fbf liste)]
                             [(neg? fbf) (fonction (fils fbf) (append fbf liste))]
                             [else (append (fonction (filsG fbf) (append fbf liste))
                                           (fonction (filsD fbf) (append fbf liste)) )])))]
    (fonction fbf '()) ))
